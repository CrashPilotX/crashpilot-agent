...
# for root (/etc/crashpilot, /opt/crashpilot/data); a moved install used to
# send the data to /root. For a root install elsewhere, these are the
# settings that point it there, one KEY=value per line.
moved_paths() {
  [[ "$CONFIG_DIR" != "/etc/crashpilot" ]] && echo "CRASHPILOT_CONFIG_DIR=$CONFIG_DIR"
  [[ "$DATA_DIR" != "/opt/crashpilot" ]] && echo "CRASHPILOT_DATA_DIR=$DATA_DIR/data"
  return 0
}

# Create a wrapper script in a system PATH location. $2 is moved_paths
# output: the wrapper defaults those settings, so commands run by hand (and
# this installer's own configure or enroll) use the services' dirs.
create_wrapper() {
  local target="$1" kv defaults=""
  while IFS= read -r kv; do
    [[ -n "$kv" ]] && defaults+=": \"\${${kv%%=*}:=${kv#*=}}\"; export ${kv%%=*}"$'\n'
  done <<< "${2:-}"
  mkdir -p "$(dirname "$target")"
  cat > "$target" << WRAPPER
#!/bin/bash
# CrashPilot wrapper: generated by install.sh
${defaults}exec "$VENV_DIR/bin/crashpilot" "\$@"
WRAPPER
  chmod +x "$target"
}
...
if [[ $EUID -eq 0 ]]; then
  ...
  create_wrapper /usr/local/bin/crashpilot "$(moved_paths)"
...
# Tell each service where a moved install keeps its config and data (see
# moved_paths; the sandboxed boot analysis cannot write under /root), in a
# drop-in, which an agent update refreshing the units leaves alone. With the
# defaults, a drop-in from an earlier run is removed.
install_paths_dropin() {
  local unit="$1" dir="/etc/systemd/system/$1.d" conf="$WORK_DIR/$1.paths.conf"
  local lines="" kv
  while IFS= read -r kv; do
    [[ -n "$kv" ]] && lines+="Environment=$kv"$'\n'
  done <<< "$(moved_paths)"
  # The boot analysis runs with /var read-only and home directories protected.
  if [[ "$DATA_DIR" != "/opt/crashpilot" && "$unit" == "crashpilot.service" ]]; then
    lines+="ReadWritePaths=-$DATA_DIR/data"$'\n'
  fi
  if [[ -z "$lines" ]]; then
...
systemd_install_consented() {
  ...
  # Installers before this one did set them up from a non-root run, and a
  # non-root run cannot remove them.
  if grep -qsF "$VENV_DIR/bin/crashpilot" /etc/systemd/system/crashpilot*.service; then
    warn "systemd units from an earlier non-root install still run $VENV_DIR as root."
    INSTALL_PROBLEMS+=("system units run this user's venv as root: re-run with sudo to replace them, or remove /etc/systemd/system/crashpilot*")
  fi
  return 1
}
